...
date: 2023-03-02
category: log
description: 用 Docker 在 OpenWrt 软路由上部署网络存储，给 Mac 做无线 Time Machine 备份；兼谈 SMR 硬盘与文件系统。
...
　　这里是一些概念的介绍，对此较为了解的读者可以直接看实践的部分。
...
　　由于磁道存在重叠，叠瓦磁盘的写入过程较为复杂。读者们只需要知道，当SMR磁盘在尝试随机读写时，会有很明显的性能瓶颈。
